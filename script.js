...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function animateMetrics() {
        metricNumbers.forEach(metric => {
            const targetText = metric.getAttribute('data-target');
    ...
    const metricsSection = document.querySelector('.metrics, .founder-stats');
    ...
    // ===== 15. CONSOLE MESSAGE (Developer Credit) =====
    ...
    // ===== 17. PRELOAD BACKGROUND IMAGE =====
    const bgImage = new Image();
    bgImage.src = 'https://images.unsplash.com/photo-1589519160732-57fc6ea83bc8?q=80&w=2070&auto=format&fit=crop';
    bgImage.onload = function() {
        // Background loaded successfully
        document.body.style.opacity = '1';
    };
    
    // ===== 18. ACTIVE STATE FOR MOBILE MENU =====
    if (mobileNav) {
        mobileNav.querySelectorAll('a').forEach(link => {
            const linkPage = link.getAttribute('href');
            if (linkPage === currentPage) {
    ...
    // ===== 20. FINAL INITIALIZATION LOG =====
    console.log('✅ All systems ready');
});

// ===== 21. WINDOW LOAD EVENTS (for things that need full page load) =====
...
// ===== 23. BROWSER BACK BUTTON HANDLING =====
...
// ===== 25. SERVICE WORKER REGISTRATION (optional - for offline capability) =====
...
// Load CMS data after page loads
document.addEventListener('DOMContentLoaded', function() {
    loadCMSData();
});

// ===== NETLIFY IDENTITY FOR ADMIN =====
